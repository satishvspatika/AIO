...
void fetchFromHttpAndUpdate(char *fileName, bool alreadyLocked) {
  ...
  while (true) {
    ...
    if (consecutive_fails >= 3) {
      ...
      // Re-init session since we hard reset the modem
...
pattern = r"void fetchFromHttpAndUpdate\(char \*fileName, bool alreadyLocked\) \{.*?\n\}\n"
replaced = re.sub(pattern, new_func.strip() + "\n", content, flags=re.DOTALL)
