...
bool load_apn_config(const char* current_ccid, char *target_apn, size_t max_len) {
  ...
  if (xSemaphoreGetMutexHolder(fsMutex) == xTaskGetCurrentTaskHandle()) {
      xSemaphoreGive(fsMutex);
  }
  ...
  if (strstr(current_ccid, ccid_buf) != NULL) {
    ...
    // [HR-H01] Carrier Guard: Protect SPIFFS.remove with fsMutex
...
bool try_activate_apn(const char *apn) {
  ...
  if (strstr(current_apn_resp, "+CGDCONT: 1,") != NULL && 
     (strstr(current_apn_resp, target_apn_quoted.c_str()) != NULL || strstr(current_apn_resp, apn) != NULL)) {
