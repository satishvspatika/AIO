...
## 🔒 PHASE 11: SLEEP-GATE ATOMIC SHIELDING & GHOST-AWAKE FIXES
*Eliminating permanent 80mA "ghost-awake" leaks and shielding Core 0 from sleep-cutting the I2C pipeline.*

* **RTC Resync Flag Leak Cured:** Sealed a critical memory leak within `rtcRead.ino`'s `resync_time()` sequence. If the RTC task failed to grab the 15-second `modemMutex` (e.g., due to an active FTP upload), it historically returned without wiping `health_in_progress`. That left the sleep-gate permanently blocked, bleeding the battery dead. We now systematically restabilize the entire suite of `health_in_progress=false` and `sync_mode=eExceptionHandled` booleans statically before exiting the function.
* **Deep-Sleep Nanosecond State Tearing:** Inoculated the OS sleep command directly against microsecond race conditions by injecting `(gprs_started == false)` into the literal Deep Sleep gate equation in `loop()`. This mathematically guarantees the ESP32 cannot physically hibernate if a background task happens to flip its busy flag one nanosecond after Core 0 initiated shutdown.
* **Holistic Task Starvation Re-Check:** Substituted the single final `if (schedulerBusy)` pre-sleep test with a sweeping `if (schedulerBusy || gprs_started || health_in_progress || httpInitiated)` holistic evaluation. Because all RTOS tasks now sit evenly at `Priority 2` (Phase 10), this prevents any horizontally scheduled sensors from accidentally unspooling their hardware cycles over the precise 50ms sliver immediately before the deep sleep command is given.

---

## 💾 PHASE 12: DMA FLASH CORRUPTION & CROSS-THREAD SAFELOCKS
*Sealing severe SPIFFS file-handle leakage during massive daily DB log instantiations and asynchronous APN recoveries.*

* **`HANDLE_NO_FILE` VFS Deadlock Guard:** Expanded the `fsMutex` RTOS blanket organically across the enormous 8:45 AM SPIFFS instantiation sequence inside `scheduler.ino`. The original code opened a `.print()` handler perfectly behind a mutex, but subsequently *dropped* the hardware lock *before* sequentially writing 90-days of gap data directly over the open buffer. Any concurrent RTC sync attempt during that ~2-second window would have definitively collided via the ESP32's DMA, fracturing the internal FAT sector completely. 
...
## 🔒 PHASE 16: SYSTEM 1/2 FTP MUTEX ESCAPE
*Closing a massive unguarded SPIFFS execution hole exclusively found in the FTP pipeline.*

* **Unguarded Daily FTP Buffer Copier (`gprs.ino`):** Uncovered a critical concurrency escape inside `send_unsent_data()` explicitly affecting `SYSTEM == 1` and `SYSTEM == 2`. The main FTP Backlog logic correctly grabbed the `fsMutex` to build its payload, and explicitly released it via `xSemaphoreGive()` immediately prior to pushing the data over the 3-minute cellular link to let the RTOS breathe. However, if the current cycle was the `09:30 AM` Daily FTP cycle, the code naturally fell through into the `sampleNo == 3` block and executed `SPIFFS.remove()` and `copyFile()` entirely **unguarded**! A concurrent Core 1 sensor write exactly during this 09:30 AM sequence would have physically obliterated the flash sector FAT indices.
* **Recursive Deadlock Unsnagged (`send_ftp_file`):** Additionally eliminated an internal recursion logic flaw where `send_ftp_file` attempted to `xSemaphoreTake(fsMutex)` a second time during its cleanup sequence even if it was explicitly passed `alreadyLocked == true`. In FreeRTOS, attempting to lock a non-recursive mutex you already hold instantly hard-locks the execution thread forever.
* **The Solution:** We systematically wrapped the entire `09:30 AM` Daily File formulation block directly inside its own explicit `xSemaphoreTake(fsMutex)` enclosure, and eliminated the double-lock attempt. The FTP Pipeline (`SYS 1/2`) is now formally as rigidly constrained as the JSON REST block.

* **UI Backlog Counter Desync (`global_functions.ino`):** The `get_total_backlogs()` iteration physically counted all records in `/unsent.txt` despite the HTTP sequence using `/unsent_pointer.txt` to logically fast-forward past already transmitted JSON payload bounds without destroying the host flash table.
* **The Solution:** `get_total_backlogs()` now explicitly loads the pointer index integer and runs a native `f.seek(ptrVal)` onto the file pointer before initiating its line-counting array logic so the internal UI variables map strictly to untransmitted queues.

* **HTTP Backlog Array Truncation (`gprs.ino`):** Uncovered a classic double-increment compiler bug inside the `SYSTEM 0` (JSON REST API) backlog loop. The iterator (`backlog_processed_count++`) was physically triggered at the top of the while-loop, and then mistakenly triggered again natively inside its exit-guard evaluation (`if (++backlog_processed_count >= 15)`). By stepping vertically by increments of 2, the 15-record maximum backlog payload dynamically artificially hard-capped itself seamlessly at exactly 8.
* **The Solution:** Sanitized the exit-guard to a pure logical evaluation (`backlog_processed_count >= 15`). The system now mathematically utilizes the physical HTTP throughput max payload (15 array entries per 3-minute cellular window) securely and flawlessly.
