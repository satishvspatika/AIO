...
void get_registration() {
  ...
  if (!isBSNL && (cgregResp.indexOf(",1") != -1 || cgregResp.indexOf(",5") != -1)) {
    ...
    if (strlen(apn_str) > 3) {
      ...
      String expected_proto = (strcmp(apn_str, "jionet") == 0) ? "IPV4V6" : "IP";
      
      if (cgdcont_resp.indexOf(apn_str) == -1 || cgdcont_resp.indexOf(expected_proto) == -1) {
          debugln("[GPRS] Pre-setting APN for CID 1: " + String(apn_str) + " (" + expected_proto + ")");
  ...
  if (is_registered) {
    ...
    if (diag_consecutive_reg_fails >= 10) {
      ...
      diag_consecutive_reg_fails = 0; // Prevent infinite persistent restart loop
      vTaskDelay(1000 / portTICK_PERIOD_MS);
      ESP.restart();
    }
  }
